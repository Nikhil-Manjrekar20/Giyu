  ...
  */
  ...
  const validateForm = () => {
    ...
    if (!formData.persons) {
      ...
      !validatePersons(parseInt(formData.persons), venue.base_members)
    ) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      extraPersonCharges: calculateExtraCharges(parseInt(formData.persons), venue.name),
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {formData.slotId && (
              <div className="bg-gray-700/30 rounded-lg p-3">
                ...
                <p className="text-white font-medium text-sm">{getSelectedSlotTime()}</p>
              </div>
            )}
            ...
            {formData.persons && (
              <div className="bg-gray-700/30 rounded-lg p-3">
                ...
                <p className="text-white font-medium">{formData.persons} Person{parseInt(formData.persons) !== 1 ? 's' : ''}</p>
                {calculateExtraCharges(parseInt(formData.persons), venue.name) > 0 && (
                  <p className="text-yellow-400 text-xs mt-1">
                    +₹{calculateExtraCharges(parseInt(formData.persons), venue.name)} extra charges
                  </p>
                )}
              </div>
            )}
